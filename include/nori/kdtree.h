...
private:
    ...
    void nearest_neighbors_impl(std::vector<T>& values, std::size_t left, std::size_t right, 
            const std::array<real,N>& p, std::size_t number, 
            float& max_distance, const Norm& norm,
            const Mesh *mesh=nullptr) const 
    {
        if (right > left) 
        {
            ...
            if (norm(difference(p,elements[median]))<max_distance 
                    && (mesh == nullptr || elements[median].mesh == mesh)) 
            {
            ...
            if ((right-left)>1) 
            {
                ...
                {//First left node and then, if needed, right node
                    nearest_neighbors_impl(values,left,median,p,number,max_distance,norm, mesh);
                    if (norm(difference(p,pplane)) < max_distance) //We still need to explore the other node
                        nearest_neighbors_impl(values,median+1,right,p,number,max_distance,norm, mesh);
...
public:
    ...
    std::vector<T> nearest_neighbors(const std::array<real,N>& p, 
            std::size_t number, float max_distance, const Norm& norm,
            const Mesh *mesh=nullptr) const 
    {
        std::vector<T> sol;
        nearest_neighbors_impl(sol,0,elements.size(),p,number,max_distance,norm, mesh);
        return sol;
    }
    

    std::vector<T> nearest_neighbors(const std::array<real,N>& p, std::size_t number = 1, 
    ...
    std::vector<T> nearest_neighbors(const P& p, std::size_t number, 
            float max_distance, const Norm& norm,
            const Mesh *mesh=nullptr) const {
        std::array<real,N> p_impl;
        for (std::size_t i = 0; i<N; ++i) p_impl[i] = p[i];
        return nearest_neighbors(p_impl,number,max_distance,norm, mesh);
    }
    
    template<typename P> //P -> position N dimensional, should have random access
    std::vector<T> nearest_neighbors(const P& p, std::size_t number = 1, 
            float max_distance = std::numeric_limits<float>::infinity(),
            const Mesh *mesh=nullptr) const {
        return nearest_neighbors(p,number,max_distance,
            [] (const std::array<real,N>& v) {
...
using PhotonMap = nn::KDTree<Photon,3,nn::PhotonPosition>;
