...
public:
     ...
     */
    void samplePosition(Sampler *sampler, Point3f &p, Normal3f &n, Point2f &uv, float &pdf, n_UINT &triangleId) const;

    //// Return the normal of the given triangle
    Normal3f getNormal(n_UINT index, const Point2f &uv) const;

    Point3f uvTo3D(n_UINT index, const Point2f &uv) const;
    Point2f uvFrom3D(n_UINT index, const Point3f &p) const;
