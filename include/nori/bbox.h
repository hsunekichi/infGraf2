...
template <typename _PointType> struct TBoundingBox {
    ...
    VectorType getUnitCubeCoordinates(const PointType &p) const {
        return (p - min).cwiseQuotient(max - min);
    }

    PointType lerp(const VectorType &coords) const {
        return min + coords.cwiseProduct(max - min);
    }
    ...
    bool rayIntersectUnoptim(const Ray3f &ray) const 
    {
    ...
    bool rayIntersectUnoptim(const Ray3f &ray, float &minT) const 
    {
        float nearT = -std::numeric_limits<float>::infinity();
        float farT = std::numeric_limits<float>::infinity();
