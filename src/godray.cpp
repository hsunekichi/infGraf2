 ...
 */
class godray : public BSDF
{
public:
    godray(const PropertyList &propList) 
    {
    ...
    void addChild(NoriObject* obj, const std::string& name = "none") {
        ...
        case ETexture:
            if (name == "albedo")
            {
                ...
                throw NoriException("Godray::addChild(<%s>,%s) is not supported!",
                classTypeName(obj->getClassType()), name);
            break;

        default:
            throw NoriException("Godray::addChild(<%s>) is not supported!",
...
NORI_REGISTER_CLASS(godray, "godray");
NORI_NAMESPACE_END
