...
/**
 * \brief DipoleScattering / Lambertian BRDF model
 */
class Dipole : public BSDF
{
public:
    Dipole(const PropertyList &propList) 
    {
    ...
    void addChild(NoriObject* obj, const std::string& name = "none") {
        ...
        case ETexture:
            if (name == "albedo")
            {
                ...
                throw NoriException("DipoleScattering::addChild(<%s>,%s) is not supported!",
                classTypeName(obj->getClassType()), name);
            break;

        default:
            throw NoriException("DipoleScattering::addChild(<%s>) is not supported!",
...
NORI_REGISTER_CLASS(Dipole, "dipole");
NORI_NAMESPACE_END
